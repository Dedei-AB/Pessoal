// =============================================================================
// models/conta.js
// Queries SQL para a tabela de conta
// =============================================================================
...
class ContaModel {
  async findAll() {
    const response = await database.query('SELECT * FROM conta ORDER BY id_conta DESC');
    return response.rows;
  }

  async findById(id) {
    const response = await database.query('SELECT * FROM conta WHERE id_conta = $1', [id]);
  ...
  async delete(id) {
    const response = await database.query('DELETE FROM conta WHERE id_conta = $1', [id]);
    return response.rowCount > 0;
  }
}

export default new ContaModel();